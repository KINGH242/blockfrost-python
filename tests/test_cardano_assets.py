...
def test_asset_utxos(requests_mock):
    api = BlockFrostApi()
    mock_data = [
        {
            "address": "addr1qxqs59lphg8g6qndelq8xwqn60ag3aeyfcp33c2kdp46a09re5df3pzwwmyq946axfcejy5n4x0y99wqpgtp2gd0k09qsgy6pz",
            ...
            "data_hash": None,
            "inline_datum": None,
            "inline_datum_json": None,
            "reference_script_hash": None
        }
    ]
    requests_mock.get(f"{api.url}/assets/{asset}/utxos", json=mock_data)
    assert api.asset_utxos(asset) == convert_json_to_object(mock_data)


def test_integration_asset_utxos():
    if os.getenv('BLOCKFROST_PROJECT_ID_MAINNET'):
        api = BlockFrostApi(project_id=os.getenv('BLOCKFROST_PROJECT_ID_MAINNET'))
        api.asset_utxos(asset)
